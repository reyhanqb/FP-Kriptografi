<template>
  <div>
    ...
    <h3 v-if="voted">Your public key is: {{ rsaKey.public_key }}</h3>
    <h3 v-if="voted">Your private key is: {{ rsaKey.private_key }}</h3>
    <button @click="submitVote" :disabled="selectedOption === null">Vote</button>
    <div>
      <h3>Enter Your Name to View Your Vote</h3>
      <div class="columns">
        <div class="column">
    ...
    <div>
      <h3>Enter NRP to Unblind Vote</h3>
      <div class="columns">
        <div class="column">
          <label for="public-key-e">Public Key (e):</label>
          <input type="text" id="public-key-e" v-model="publicKey.e">
        </div>
        <div class="column">
          <label for="public-key-n">Public Key (n):</label>
          <input type="text" id="public-key-n" v-model="publicKey.n">
        </div>
      </div>
      <div class="columns">
        <div class="column">
          <label for="private-key-d">Private Key (d):</label>
          <input type="text" id="private-key-d" v-model="privateKey.d">
        </div>
        <div class="column">
          <label for="private-key-n">Private Key (n):</label>
          <input type="text" id="private-key-n" v-model="privateKey.n">
        </div>
      </div>
      <button @click="unblindVote">Unblind Vote</button>
      <p v-if="showUnblindedVote">{{ unblindedVoteMessage }}</p>
    </div>
    <div v-if="voted">
    ...
    <div>
      ...
      <input type="text" v-model="blindedVote">
    </div>
  </div>
</template>
...
submitVote() {
  ...
  console.log(this.rsaKey.public_key);
  console.log(this.rsaKey.private_key);
...
generateKeyPair() {
  ...
  const private_key = JSON.stringify(privateKey);

  return { p, q, publicKey, privateKey, public_key, private_key };
},
...
generateBlindSignature() {
  ...
  const publicKey = this.rsaKey.publicKey;
  
  // Calculate the blind signature using the voter's private key and public key
